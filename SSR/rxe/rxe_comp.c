					...
					struct rxe_send_wqe *wqe)
{
	 ...
	 */
	diff = psn_compare(lower_32_bits(pkt->psn), wqe->last_psn);
	if (diff > 0) {
	...
	if (diff < 0) {
		 ...
		 */
		if (lower_32_bits(pkt->psn) == wqe->last_psn)
					...
					struct rxe_send_wqe *wqe)
{
	...
	case IB_OPCODE_RC_RDMA_READ_RESPONSE_MIDDLE:
		if (pkt->opcode != IB_OPCODE_RC_RDMA_READ_RESPONSE_MIDDLE &&
			 ...
			 */
			if ((lower_32_bits(pkt->psn) == wqe->first_psn &&
			     pkt->opcode ==
	...
	case IB_OPCODE_RC_ACKNOWLEDGE:
		...
		case AETH_RNR_NAK:
			...
			rxe_qp_counter_inc(qp, RXE_CNT_RCV_RNR);
...
static void do_complete(struct rxe_qp *qp, struct rxe_send_wqe *wqe)
{
	...
	if (wqe->wr.opcode == IB_WR_SEND ||
		...
		rxe_qp_counter_inc(qp, RXE_CNT_RDMA_SEND);
	}
					   ...
					   struct rxe_send_wqe *wqe)
{
	if (pkt && wqe->state == wqe_state_pending) {
		if (psn_compare(wqe->last_psn, lower_32_bits(qp->comp.psn)) >= 0) {
			qp->comp.psn = (((qp->comp.psn >> 32) << 32) +
					wqe->last_psn + 1) & BTH_PSN_MASK;
			qp->comp.opcode = -1;
		}
...
int rxe_completer(void *arg)
{
	...
	while (1) {
		...
		case COMPST_WRITE_SEND:
			if (wqe->state == wqe_state_pending &&
			    wqe->last_psn == lower_32_bits(pkt->psn))
		...
		case COMPST_ERROR_RETRY:
			...
			if (qp->comp.retry_cnt > 0) {
				...
				if (psn_compare(qp->req.psn,
					 ...
					 */
					rxe_counter_inc(rxe, RXE_CNT_COMP_RETRY);
					rxe_qp_counter_inc(qp,
							   RXE_CNT_COMP_RETRY);
					qp->req.need_retry = 1;
				...
				rxe_qp_counter_inc(qp, RXE_CNT_RETRY_EXCEEDED);
