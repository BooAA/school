...
static int hdr_check(struct rxe_pkt_info *pkt)
{
	...
	if (qpn != IB_MULTICAST_QPN) {
		index = (qpn == 1) ? port->qp_gsi_index : rxe_vqpn_to_index(qpn);
...
void rxe_rcv(struct sk_buff *skb)
{
	...
	if (pkt->mask & RXE_REQ_MASK)
		err = rxe_mac_check(skb, pkt, &pkt->qp->resp_ctx);
	else
		err = rxe_mac_check(skb, pkt, &pkt->qp->comp_ctx);

	if (unlikely(err))
		goto drop;

	if (pkt->mask & RXE_REQ_MASK)
		err = rxe_cipher_decrypt(&pkt->qp->resp_cipher_ctx, 
					payload_addr(pkt), payload_size(pkt));
	else
		err = rxe_cipher_decrypt(&pkt->qp->comp_cipher_ctx, 
					payload_addr(pkt), payload_size(pkt));
	...
	rxe_qp_counter_inc(pkt->qp, RXE_CNT_RCVD_PKTS);
