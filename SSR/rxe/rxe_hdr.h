...
#define RXE_CIPHER_KEY_SIZE	(32)
#define RXE_MAC_SIZE		(32)
#define RXE_MAC_KEY_SIZE	(32)
#define RXE_ICRC_SIZE		(RXE_MAC_SIZE + 4)
#define RXE_MAX_HDR_LENGTH	(80)

extern u8 mac_key_kobj[RXE_MAC_KEY_SIZE];
...
#define BTH_ACK_MASK		(0x8000000000000000)
#define BTH_RESV7_MASK		(0x7f00000000000000)
#define BTH_PSN_MASK		(0x00ffffffffffffff)
...
static inline int __bth_ack(void *arg)
{
	...
	return 0 != (cpu_to_be64(BTH_ACK_MASK) & bth->apsn);
}
...
static inline void __bth_set_ack(void *arg, int ack)
{
	...
	if (ack)
		bth->apsn |= cpu_to_be64(BTH_ACK_MASK);
	else
		bth->apsn &= ~cpu_to_be64(BTH_ACK_MASK);
}
...
static inline void __bth_set_resv7(void *arg)
{
	...
	bth->apsn &= ~cpu_to_be64(BTH_RESV7_MASK);
}

static inline u64 __bth_psn(void *arg)
{
	struct rxe_bth *bth = arg;

	return BTH_PSN_MASK & be64_to_cpu(bth->apsn);
}

static inline void __bth_set_psn(void *arg, u64 psn)
{
	struct rxe_bth *bth = arg;
	u64 apsn = be64_to_cpu(bth->apsn);

	bth->apsn = cpu_to_be64((BTH_PSN_MASK & psn) |
			(~BTH_PSN_MASK & apsn));
}
...
static inline u64 bth_psn(struct rxe_pkt_info *pkt)
{
	return __bth_psn(pkt->hdr);
}

static inline void bth_set_psn(struct rxe_pkt_info *pkt, u64 psn)
{
