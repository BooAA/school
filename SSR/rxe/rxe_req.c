	...
	for (wqe_index = cons; wqe_index != prod;
		...
		if (first) {
			...
			if (mask & WR_WRITE_OR_SEND_MASK) {
				npsn = (lower_32_bits(qp->comp.psn) - 
				        wqe->first_psn) &BTH_PSN_MASK;
			   ...
			   int payload)
{
	...
	if (pkt->mask & RXE_START_MASK) {
		wqe->first_psn = lower_32_bits(qp->req.psn);
		wqe->last_psn = lower_32_bits((qp->req.psn + num_pkt - 1) 
						& BTH_PSN_MASK);
	}

	if (pkt->mask & RXE_READ_MASK)
		qp->req.psn = (((qp->req.psn >> 32) << 32) + 
			       wqe->first_psn + num_pkt) & BTH_PSN_MASK;
	else
...
next_wqe:
	...
	if (payload > mtu) {
		if (qp_type(qp) == IB_QPT_UD) {
			...
			wqe->first_psn = lower_32_bits(qp->req.psn);
			wqe->last_psn = lower_32_bits(qp->req.psn);
			qp->req.psn = (qp->req.psn + 1) & BTH_PSN_MASK;
