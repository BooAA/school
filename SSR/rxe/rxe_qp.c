			     ...
			     struct ib_qp_init_attr *init)
{
	...
	qpn			= qp->pelem.index;
	port			= &rxe->port;
	vqpn                    = rxe_vqpn_add(qpn);

	rxe_create_qp_kobj(qp, vqpn);
		     ...
		     struct ib_udata *udata)
{
	...
	rxe_qp_init_misc(rxe, qp, init);
        if (strlen(mac_key_kobj)) {
                memcpy(qp->mac_key, mac_key_kobj, RXE_MAC_KEY_SIZE);
                pr_info("Key of qpn = %u:\n", qp->ibqp.qp_num);
                for (i = 0; i < RXE_MAC_KEY_SIZE / sizeof(u32); i++) {
                        pr_info("+ key[%3d:%3d] = 0x%08x\n", 
                                i * 32 + 31, i * 32, 
                                *(((u32 *)(qp->mac_key)) + i));
                }
        }
		     ...
		     struct ib_udata *udata)
{
	...
	if (mask & IB_QP_AV) {
		...
		err = rxe_rctl_conn_cnt_inc(qp);
		if (err == -1) {
			pr_warn_ratelimited("too much connection to %pI4\n",
				&qp->pri_av.dgid_addr._sockaddr_in.sin_addr.s_addr);
			return err;
		}
	}
	...
	if (mask & IB_QP_RQ_PSN) {
		...
		pr_debug("qp#%d set resp psn = 0x%llx\n", qp_num(qp),
			 qp->resp.psn);
	}
	...
	if (mask & IB_QP_SQ_PSN) {
		...
		pr_debug("qp#%d set req psn = 0x%llx\n", qp_num(qp), 
		         qp->req.psn);
	}
