...
{
	s64 diff = psn_compare(pkt->psn, qp->resp.psn);
	...
	case IB_QPT_RC:
		if (diff > 0) {
			...
			rxe_qp_counter_inc(qp, RXE_CNT_OUT_OF_SEQ_REQ);
			return RESPST_ERR_PSN_OUT_OF_SEQ;

		} else if (diff < 0) {
			rxe_counter_inc(rxe, RXE_CNT_DUP_REQ);
			rxe_qp_counter_inc(qp, RXE_CNT_DUP_REQ);
				    ...
				    struct rxe_pkt_info *pkt)
{
	...
	if (wc->status == IB_WC_SUCCESS) {
		...
		rxe_qp_counter_inc(qp, RXE_CNT_RDMA_RECV);
...
static struct resp_res *find_resource(struct rxe_qp *qp, u64 psn)
{
	int i;
...
{
	enum resp_states rc;
	u64 prev_psn = (qp->resp.ack_psn - 1) & BTH_PSN_MASK;
	...
	while (1) {
		...
		case RESPST_ERR_RNR:
			if (qp_type(qp) == IB_QPT_RC) {
				...
				rxe_qp_counter_inc(qp, RXE_CNT_SND_RNR);
				/* RC - class B */
