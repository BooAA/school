			 ...
			 struct ib_udata *udata)
{
	...
	err = rxe_rctl_qp_cnt_inc(qp);
	if (err == -1){
		pr_warn_ratelimited("too much qp for pid %i\n", qp->pid);
		return err;
	}
...
static int rxe_destroy_qp(struct ib_qp *ibqp, struct ib_udata *udata)
{
	...
	rxe_vqpn_remove(ibqp->qp_num);

	rxe_rctl_qp_cnt_dec(qp);
	rxe_rctl_conn_cnt_dec(qp);
